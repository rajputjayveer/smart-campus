...
export default function ShopkeeperCouponManagement({ showToast }) {
    ...
    const loadCoupons = async () => {
        try { setLoading(true); const data = await api.getShopkeeperCoupons(); setCoupons(Array.isArray(data) ? data : []); }
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            showToast?.success?.('Shop coupon created'); setFormData(initialForm); setShowForm(false); loadCoupons();
    ...
    return (
        <div className="space-y-5">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-extrabold text-gray-900">Shop Coupons</h2>
                    <p className="text-sm text-gray-400">These offers apply only to your stall</p>
                </div>
            ...
            {showForm && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-5 slide-up">
                    <h3 className="text-base font-bold text-gray-800 mb-4">Create Shop Coupon</h3>
                    <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" required value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value.toUpperCase() })}
                                className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm bg-gray-50 focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 font-mono font-bold tracking-wider" placeholder="STALL20" />
                        </div>
                        <div>
            ...
            {loading ? (
                ...
                <div className="text-center py-16">
                    ...
                    <p className="text-lg font-semibold text-gray-400">No shop coupons yet</p>
                    <p className="text-gray-300 text-sm mt-1">Create your first coupon to attract customers</p>
                </div>
            ) : (
